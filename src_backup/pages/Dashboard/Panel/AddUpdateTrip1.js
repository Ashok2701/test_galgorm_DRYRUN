...
class AddUpdateTrip1 extends React.Component {
    ...
    drop(event, eventType) {
        ...
        /*
                ...
                */
        console.log("T111 inside drag eventtype =", eventType);
        if (eventType === 'createVehicle') {
            ...
            if (!(this.props.trips && this.props.trips[0] && (this.props.trips[0].lock || this.props.trips[0].tmsValidated))) {
                ...
                if (type === 'vehicle') {
                    ...
                    if (data.type === "open" && data.dlvystatus == "8") {
                    ...
                    if (trip.code === "") {
                        ...
                        status = false;
                        this.setState({
                            errorMessage: "The vehicle is missing, please drag and drop it first.",
                            addAlertShow: true,
                            error: true,
                        })
                        ...
                        if (trip.alldrivers === 2) {
                            ...
                            if (trip.driverslist && !trip.driverslist.includes(data.driverid)) {
                                status = false;
                                this.setState({
                                    errorMessage: "The selected driver is not associated with the trip vehicle",
                                    addAlertShow: true,
                                    error: true
                                });
                            }
                            ...
                            if (trip.drops > 0) {
                                ...
                                for (var i = 0; i < trip.dropObject.length; i++) {
                                    if ((trip.dropObject[i].trailer).trim() === '' || trip.dropObject[i].trailer === data.trailer) {
                                        ...
                                        status = false;
                                        this.setState({
                                            errorMessage: "Route documents - The trailer is not compatible with the added trailer.",
                                            addAlertShow: true,
                                            error: true,
                                    ...
                                    if (trip.drops > 0) {
                                        for (var i = 0; i < trip.dropObject.length; i++) {
                                            if (data.tclcod && data.tclcod.includes(trip.dropObject[i].products.productCateg)) {
                                                ...
                                                status = false;
                                                this.setState({
                                                    errorMessage: "Route documents - products are not compatible with the added trailer",
                                                    addAlertShow: true,
                                                    error: true,
                    ...
                    if (docsCompatability === true) {
                        if (trip.trailerLink && trip.trailerLink != 'Yes') {
                            ...
                            this.setState({
                                errorMessage: "The association between trailer type and vehicle class does not exist",
                                addAlertShow: true,
                                error: true
                            });
                        }
    ...
    addDrop = (currentTrip, data, trip) => {
        ...
        if (dropCompatability == true) {
            ...
            if (trip.allcustomers === 2) {
                ...
                if (trip.customerlist && !trip.customerlist.includes(data.bpcode)) {
                    ...
                    this.setState({
                        errorType: 'customer',
                        errorMessage: "The customer is not associated with the trip vehicle",
                        addAlertShow: true,
                        error: true
                    });
        ...
        if (dropCompatability == true) {
            ...
            if (error === 'product') {
                this.setState({
                    errorMessage: "Selected products are not compatible with the vehicle's product category",
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: "The document already contains a trailer, but does not correspond to a trip trailer.",
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: 'The selected products are not compatible with the trailer product category.',
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: "The document already contains the driver, but it does not match with trip Driver.",
                    addAlertShow: true,
                    error: true
                });
            }
                ...
                this.setState({
                    errorMessage: "The customer is not associated with the current vehicle",
                    addAlertShow: true,
                    error: true
                });
            }
    ...
    addPickup = (currentTrip, data, trip) => {
        ...
        if (pickupCompatability) {
            ...
            if (pickupError === 'product') {
                ...
                this.setState({
                    errorMessage: "The document already contains the driver, but it does not match the driver of the trip.",
                    addAlertShow: true,
                    error: true
                });
            }
    ...
    onConfirmClick = (trip) => {
        ...
        if (trip.driverId !== "") {
            ...
            if (trip.vol <= (dropVolume + pickupVolume) ||
                ...
                if (trip.vol <= (dropVolume + pickupVolume)) {
                    message = message + `Trip Volume (${dropVolume + pickupVolume}  M3) exceeds the maximum allowed vehicle volume capacity(${trip.vol} M3))`;
                }
                if (trip.vehicleObject.maxordercnt <= (trip.pickupObject.length + trip.dropObject.length)) {
                    message = message + `No of Trips(${trip.pickupObject.length + trip.dropObject.length}) is greater than the maximum number of vehicle trips(${trip.vehicleObject.maxordercnt})) for the selected date.`;
                }
                if (trip.vehicleObject.capacities <= (pickupCapacities + dropCapacities)) {
                    message = message + `Trip Weight (${pickupCapacities + dropCapacities} KG) is greater than the maximum allowed vehicle weight capacity(${trip.vehicleObject.capacities} KG))`;
                }
                ...
                if (trip.pickupObject.length <= 0 && trip.dropObject.length <= 0) {
                    message = "Pick-up or drops not assigned to the trip. Are you sure you want to delete the trip ?";
                    warn = false;
                }
            ...
            return (
                <table>
                    ...
                    <tr>
                        ...
                        <td>{this.getTotalQty(this.props.selectedTripData.products, 'd')} </td>
                    </tr>
                </table>
            );
        }
    ...
    getTotalQty = (prodList, from) => {
        ...
        if (from == 'd')
            ...
            return parseFloat(TotQty).toFixed(2);
    }
    ...
    unlockTrip = (tmsValidated) => {
        if (tmsValidated) {
            this.setState({
                errorMessage: "The trip is already validated, you cannot unlock the trip.",
                addAlertShow: true
            });
        } else {
