...
import { fetchPanel, ConfirmLVS, ToLotDetailsFetchData, ToLocationsFetchData } from '../../service';
  ...
  */
  ...
  GrouplockTrips = () => {
    ...
    for (let trip in tripsPanel) {
      if (trip.optistatus === 'Open' && trip.optistatus === "Optimized") {
        trip.lock = true;
  ...
  existingTrip.tot_capacity= fld_tot_capacity;
            ...
            existingTrip.uom_volume= fld_uom_volume;
             existingTrip.generatedBy= 'AutoOpScheduler';
           existingTrip.vehicleObject= vehobj;
            ...
            existingTrip.drops= drops;
             ...
             existingTrip.weightPercentage= percentageMass;
                       ...
                       existingTrip.totalWeight= totalWeight + " " + wei_unit;
                       existingTrip.totalVolume= totalVolume + " " + vol_unit;
...
console.log("Tyyy index  tripslest  is", trippsPanel)
    ...
    if (DocCount > 0) {
      ...
      fetch('https://maps.tema-systems.com', {
        ...
        if (res.routes.length > 0) {
          ...
          this.setState({
            errorMessage: 'Trips are not genereated , Due to Documents are Not In Range',
            loader: false,
            addAlertShow: true,
          })
        }
  ...
  getRouteSchedulerApp = (routesSchedule, optimisedindex, auto, from, Tripsdata, SelectedI) => {
    var data = [];
    var newGeoData = [];
    if (auto) {
