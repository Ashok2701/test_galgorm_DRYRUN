...
class ExternalDragDrop extends Component {
    ...
    onSchActionComplete(event) {
        ...
        this.props.setcurrentView(this.scheduleObj.currentView);
    ...
    drop(event, eventType, args: DragAndDropEventArgs) {
        ...
        if (data.type === 'open' && (data.dlvystatus === '0' || data.dlvystatus === '8')) {
    ...
    onEventRendered(args) {
        // Modify the row height dynamically after events are rendered
        const rowElements = document.querySelectorAll('.e-timelines td');  // Select all timeline day rows

        rowElements.forEach((row) => {
            const currentHeight = row.offsetHeight;  // Get current row height
            row.style.height = `${currentHeight + 50}px`;  // Add 20px to the height of each row
        });
    }
                ...
                <div className='control-wrapper drag-sample-wrapper'>
                    <div className="schedule-container">
                        <ScheduleComponent
                            ...
                            eventSettings={{
                                dataSource: TripsDocData, template: this.eventTemplate.bind(this), enableMaxHeight: false,
                                fields: {
                                    id: 'docnum',
                                    subject: { name: 'bpcode' },
                                    isAllDay: false,
                            ...
                            onEventRendered={this.onEventRendered}  // Event handler to modify row height
                        >
                            <ResourcesDirective>
                            ...
                            <ViewsDirective>
                                ...
                                <ViewDirective option='TimelineWorkWeek' displayName='Week' workDays={[1, 2, 3, 4, 5, 6, 0]} allowVirtualScrolling={true} />
                            </ViewsDirective>
