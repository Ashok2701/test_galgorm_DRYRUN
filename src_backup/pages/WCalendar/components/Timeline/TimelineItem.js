...
const TimelineItem = ({ data }) => {
  ...
  const getStatus = (code, docnum) => {
    let POD_Link =
      "http://125.18.84.157:8089/scriptcase/app/TMS_LIVE/POD_EXTERNAL/?SHIPID=" +
      docnum;
  ...
  if (data.type === "PRECEIPT") {
    url =
      `${process.env.REACT_APP_X3_URL}/$sessions?f=GESXX10CPTH/2//M/` +
      data.custid;
  } else if (data.type === "DEP-SITE" || data.type === "RETURN-SITE") {
    url =
      `${process.env.REACT_APP_X3_URL}/$sessions?f=GESFCY/2//M/` +
      data.custid;
  } else if (data.type === "DLV") {
    url =
      `${process.env.REACT_APP_X3_URL}/$sessions?f=GESSDH/2//M/` +
      data.custid;
  } else if (data.type === "PICK") {
    url =
      `${process.env.REACT_APP_X3_URL}/$sessions?f=GESPRH2/2//M/` +
      data.custid;
  } else {
  }
