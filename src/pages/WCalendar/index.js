...
class WCalendar extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/report/usrsites?user=` +
          user.username
      ),
      fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/report/tripslist?date=` +
          date
      ),
    ])
      .then(([sites, mapTrips]) => {
  ...
  onMapDateAndSiteChange = (seldate, sites) => {
    Promise.all([
      fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/report/tripslistbyDateandSite?date=${seldate}&site=${sites}`
      ),
    ])
      .then(([trips]) => {
  ...
  onMapDateChange = (seldate, camefrom) => {
    ...
    if (this.state.selectedSitesArr.length > 0 && camefrom !== "uncheck") {
      ...
      Promise.all([
        fetch(
          `${process.env.REACT_APP_API_URL}/api/v1/report/tripslist?date=${seldate}`
        ),
      ])
        .then(([trips]) => {
          return Promise.all([trips.json()]);
  ...
  OnSiteSelection = (selSite, from) => {
    ...
    Promise.all([
      fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/report/vehicle?site=${selSite}`
      ),
      fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/report/route?site=${selSite}&start=${startDate}&end=${endDate}`
      ),
    ])
      .then(([vehicles, data]) => {
  ...
  RefreshDataforWeek = () => {
    ...
    if (this.state.selectedSite) {
      Promise.all([
        fetch(
          `${process.env.REACT_APP_API_URL}/api/v1/report/route?site=${this.state.selectedSitesArr}&start=${startDate}&end=${endDate}`
        ),
      ])
        .then(([data]) => {
