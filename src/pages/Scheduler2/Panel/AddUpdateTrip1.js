...
class AddUpdateTrip1 extends React.Component {
            ...
            if (!(this.props.trips && this.props.trips[0] && (this.props.trips[0].lock || this.props.trips[0].tmsValidated))) {
                ...
                if (type === 'vehicle') {
                    ...
                    if (this.props.checkedTrip && !(eventType === 'updateVehicle')) {
                        this.setState({
                            checkedTripShow: true,
                            vehicleMessage: 'Are you sure you want to replace the existing Vehicle',
                            enableOk: false,
                    ...
                    if (data.type === "open" && data.dlvystatus == "8") {
                        ...
                        if (data.pairedDoc != undefined && data.pairedDoc != '' && data.pairedDoc != ' ') {
                            ...
                            this.props.disableDivs(0, "doc", data.docnum);
                        }
                    }
                } else if (type === 'pickup') {
                    ...
                    console.log("insdie type = driver");
                    if (currentTrip.length > 0) {
                        console.log("insdie type = driver 2");
                        trip = currentTrip[0];
                        currentTrip = [];
                    }

                    let DriverCompatability = true;
                    console.log("insdie type = driver trip ", trip);
                    if (trip.code === "") {
                        console.log("insdie type = driver trip code = '' ");
                        ...
                        console.log("insdie type = driver else trip code");
                        /*
                       ...
                       //Vehicle level validation
                        ...
                        if (trip.alldrivers === 2) {
                            console.log("insdie type = driver - alldrivers if")
                            ...
                            console.log("insdie type = driver - alldrivers else")
                            if (trip.driverslist && !trip.driverslist.includes(data.driverid)) {
                                console.log("insdie type = driver - alldrivers else  if")
                                status = false;
                                this.setState({
                                    errorMessage: 'Driver & Vehicle association doesnot exist',
                                    addAlertShow: true,
                                    error: true
                                });
                            }
                            else {
                                console.log("insdie type = driver - alldrivers else else")
                                trip.driverId = data.driverid;
                                trip.driverName = data.driver;
                                currentTrip.push(trip);
                                this.props.updateTrip(currentTrip);
                    ...
                    console.log("insdie type = driver - end")
                }
    ...
    addDrop = (currentTrip, data, trip) => {
        ...
        if (trip.trailers > 0) {
            ...
            if (trip.vehicleObject.trailer && trip.vehicleObject.trailer != '') {
                ...
                if (trip.trialerObject.length === 2) {
                    triptriler2 = trip.trialerObject[1].trailer;
                }
            }
            else {
                triptriler1 = trip.trialerObject.length > 0 && trip.trialerObject[0].trailer;
                if (trip.trialerObject.length === 2) {
                    triptriler2 = trip.trialerObject[1].trailer;
                }
            }
        ...
        if (trip.trailers > 0 && trip.vehicleObject.trailerLink === "Yes") {
            if (dropCompatability === true) {

                if (trip.trialerObject.length > 0 && (trip.trialerObject[0].allproducts === 2 || trip.trialerObject[0].allproducts === '2')) {
                    ...
                    if (trip.trailers === 2 && (trip.trialerObject[1].allproducts === 2 || trip.trialerObject[1].allproducts === '2')) {
                        ...
                        for (var i = 0; i < trip.trailers; i++) {
                            ...
                            for (var j = 0; j < data.products.length; j++) {
                                if (trip.trialerObject && trip.trialerObject.length > 0 && trip.trialerObject[i].tclcod &&
    ...
    onConfirmClick = (trip) => {
        ...
        if (trip.driverId !== "" && trip.driverId !== 'null') {
            // check trailer compatability
            if (trip.vehicleObject.trailerLink === "Yes") {

                if (trip.trialerObject.length === 0) {
                    message = message + `No Trailer exist for the Trip.Please Add Trailer as Vehicle is Multi unit`;
    ...
    getTotalData = (selectedTrips, type) => {
        ...
        if (type === 'w')
            return parseFloat(tw).toFixed(2) + ' ' + wu
        else if (type === 'v')
            return parseFloat(tv).toFixed(2) + ' ' + vu
        else if (type === 'q')
            return tq + ' UN'
        else
            return '0'
