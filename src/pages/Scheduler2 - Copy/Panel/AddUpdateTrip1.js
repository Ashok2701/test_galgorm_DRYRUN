    ...
    if(trip.driverId !== ""){
        ...
        if (trip.vol <= (dropVolume + pickupVolume) ||
            ...
            if (trip.vehicleObject.maxordercnt <= (trip.pickupObject.length + trip.dropObject.length)) {
                message = message + `Trip Count (${trip.pickupObject.length + trip.dropObject.length}) exceeds the maximum allowed Vehicle Count(${trip.vehicleObject.maxordercnt}))`;
            }
            if (trip.vehicleObject.capacities <= (pickupCapacities + dropCapacities)) {
                message = message + `Trip Weight (${pickupCapacities + dropCapacities} KG) exceeds the maximum allowed Vehicle weight Capacity(${trip.vehicleObject.capacities} KG)`;
            }
        ...
        prods && prods.length > 0 && prods.map((prod) => {
         console.log("T555 insdie getTotlQty convQty",prod.convQty);
           if(prod.convQty !== undefined && prod.convQty)
           {
             ...
             console.log("T555 inside prod qty prodlist",prod.quantity);
   
        });
          console.log("total qty =",TotQty)
         let Qtywithuom = parseFloat(TotQty).toFixed(2) + " "+uom;
                console.log("total qty =",parseFloat(TotQty).toFixed(2))
                if(from == 'd')
                    return Qtywithuom;
                else if(from == 'u')
                     return uom;
