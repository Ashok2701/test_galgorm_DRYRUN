...
export default function MyMarker(props) {
  ...
  const getlink = (type, id) => {
    if (type === 'PRECEIPT') {
      url = `${process.env.REACT_APP_X3_URL}/$sessions?f=GESXX10CPTH/2//M/` + id
      return (
        <a target="_blank" href={url}>{id}</a>
      )
    }
    else if (type === "DEP-SITE" || type === "RETURN-SITE") {
      url = `${process.env.REACT_APP_X3_URL}/$sessions?f=GESFCY/2//M/` + id
      return (
        <a target="_blank" href={url}>{id}</a>
      )
    }
    else if (type === "DLV") {
      url = `${process.env.REACT_APP_X3_URL}/$sessions?f=GESSDH/2//M/` + id
      return (
        <a target="_blank" href={url}>{id}</a>
      )
    }
    else if (type === 'PICK') {
      url = `${process.env.REACT_APP_X3_URL}/$sessions?f=GESPRH2/2//M/` + id
      return (
        <a target="_blank" href={url}>{id}</a>
      )
